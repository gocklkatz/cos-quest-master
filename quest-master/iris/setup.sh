...
# Run AFTER `docker compose up -d` on a fresh volume.
...
# ── 2. Change _SYSTEM password ────────────────────────────────────────────────
echo "Setting _SYSTEM password..."
docker exec -i "$CONTAINER" bash -c 'iris session IRIS -U %SYS' <<OSEOF
Set sc = ##class(Security.Users).ChangePassword("_SYSTEM", "SYS", "$NEW_PASSWORD")
If \$$$ISERR(sc) { Write "Password already changed or error: " _ \$System.Status.GetErrorText(sc), ! }
Else { Write "Password set to: $NEW_PASSWORD", ! }
Halt
OSEOF

# ── 3. Deploy QuestMaster REST class ─────────────────────────────────────────
echo "Deploying QuestMaster.REST.Execute..."
docker cp "$(dirname "$0")/QuestMaster.REST.Execute.cls" "$CONTAINER:/tmp/"
...
# ── 4. Register /api/quest/ web application ───────────────────────────────────
echo "Registering /api/quest/ web application..."
docker exec -i "$CONTAINER" bash -c "iris session IRIS -U %SYS" <<OSEOF
Set appName = "/api/quest/"
Set exists = ##class(Security.Applications).Exists(appName)
If exists {
  Write "Web app already registered — skipping.", !
} Else {
  ...
  If \$$$ISERR(sc) { Write "ERROR: " _ \$System.Status.GetErrorText(sc), ! }
  Else { Write "Web app registered: " _ appName, ! }
}
Halt
OSEOF
